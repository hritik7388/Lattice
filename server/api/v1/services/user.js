...
//   
  ...
  paginateSearchAdmin: async (validatedBody) => {
    let query = {status: { $ne: status.DELETE },userType: { $eq: "ADMIN" },otpVerification: true, };
    const { search, fromDate, toDate, page, limit, status1, userType,} = validatedBody;
    if (search) {
      query.$or = [
        { fullName: { $regex: search, $options: "i" } },
  ...
  userPsychiatristsList: async (validatedBody) => {
    let query = { status: { $ne: status.DELETE }, userType: userType.PSYCHIATRIST };
    const { search, fromDate, toDate, page, limit } = validatedBody;
    if (search) {
      query.$or = [
        { name: { $regex: search, $options: "i" } },
